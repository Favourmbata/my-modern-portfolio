...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("favourmbata520@gmail.com");
    setCopied(true);
  }
  return (
    <div
      className={cn(
        "row-span-1 relative overflow-hidden rounded-3xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none  justify-between flex flex-col space-y-4 border-white-100/[0.1]  ",
      ...
      <div className={`${id === 6 && 'flex justify-center'} h-full`}>
        ...
        {id === 3 && (
          <div className=" flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
            ...
            <div className="flex flex-col gap-3 lg:gap-8">
            <span className="py-4 rounded-lg text-center bg-[#10132e]"/>
              {['Github' ,'VueJs', 'TaiwindCss'].map((item)=> (
                <span key={item} className="py-2 lg:py-4 lg:px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E] ">
                  {item}
                </span>
              ))}
